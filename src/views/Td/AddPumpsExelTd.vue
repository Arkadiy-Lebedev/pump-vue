...
import { itemPumpForExel } from "@/services/itemPumpForExelTd"
...
const example = async (data) => {
  ...
  await Promise.all(data.map(item => {
    ...
    if (item.id > 0) {
      return axios.post(`${API}api/pump-td/update.php`, formData)
    } else {
      return axios.post(`${API}api/pump-td/create.php`, formData)
    }
      ...
      <template #content>
        <div class="download__wrapper mb-2">
          ...
          <div class="" id="download-file">
            <div>
              <p class="mb-3 ">Загрузите файл Excel для насосов серии TD:</p>
